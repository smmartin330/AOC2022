...
class Puzzle():
    def __init__(self,input_text):
        ...
        self.crt = ['','','','','',''] # CRT is 40 wide, 6 high
        self.cycle = 0
        self.x = 1
    ...
    def do_cycle(self):
        self.cycle += 1
        sprite = list(range(self.x-1,self.x+2))
        row = (self.cycle - 1) // 40
        pixel_x = (self.cycle - 1 ) % 40
    ...
    def addx(self,x):
        this_instruction = 0
        this_instruction += self.do_cycle()
        this_instruction += self.do_cycle()
        self.x += x
    ...
    def p1(self):
        ...
        for instruction in self.input_list:
            ...
            match instruction[0]:
                case "addx":
                    self.p1_solution += self.addx(int(instruction[1]))
                case "noop":
                    self.p1_solution += self.noop()
        
    def p2(self):
        self.p2_solution = '''\n'''
        for row in self.crt:
            self.p2_solution += row
            self.p2_solution += '\n'
